...
class PMQ : public GeoCtnIntf {
 public:
  PMQ(int argc, char *argv[], int _refLevel = 8);
...
pma_seg_it PMQ::find_elt_pma(const uint64_t code_min, const uint64_t code_max, const pma_seg_it &seg) const {
 ...
 *
 */
inline code_t PMQ::get_parent_quadrant(const region_t &region) const {
...
class PMQSequential : public PMQ {
 public:
  PMQSequential(int argc, char *argv[]) : PMQ(argc, argv) {
  };
...
class PMQBinary : public PMQ {
 public:
  PMQBinary(int argc, char *argv[], int refLevel = 8) : PMQ(argc, argv, refLevel) {
  };
