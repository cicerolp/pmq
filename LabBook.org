...
#+SEQ_TODO: TODO RUNNING ANALYSIS | FINISHED CANCELED(@)
#+TAGS: @JULIO(J) @CICERO(C)
#+TAGS: IMPORTANT ignore(i) TEST(t) DEPRECATED(d) noexport(n) export(e) paper(p)
#+CATEGORY: TwitterVis
...
**** DONE [2017-09-15 Sex] Experiment =bench_insert_and_scan= :paper:exp20170919161448:
***** Setup                                                    :noexport:
- [X] Create a direcorty for this experiment 
  #+begin_src sh :results output :exports both :session template
set -e
